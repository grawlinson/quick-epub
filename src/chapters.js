...
const generateChapter = (chapter, appendChapterTitles = false, language = 'en') =>
  new Promise((resolve, reject) => {
    ...
    if (typeof chapter !== 'object') {
      ...
      return reject(new Error(`appendChapterTitles is not a boolean: ${typeof appendChapterTitles}`))
...
const generateChapters = data => new Promise((resolve, reject) => {
  ...
  .then(html => {
    ...
    data.chapters.map((chapter, index) => {
      ...
      chapter.name = `${padStart((index + 1).toString(), maxLength, '0')}_${uslug(chapter.title)}.xhtml`
